"""
Formation: 16 agents are tasked to position themselves equally spread out in a equilateral triangle
...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        world.dim_c = 2
        num_agents = 16
        num_landmarks = 4  # 正方形的四个顶点
        world.collaborative = True
        self.edge_length = 1.5  # 0.9 for 8 agents
        ...
        interval_length = self.edge_length / (num_agents / 4 + 1)

        lower_location = np.array([np.array([(i + 1) * interval_length, 0]) for i in range(int(num_agents / 4))])
        upper_location = np.array([0, self.edge_length]) + lower_location

        left_location = np.array([np.array([0, (i + 1) * interval_length]) for i in range(int(num_agents / 4))])
        right_location = np.array([self.edge_length, 0]) + left_location

        all_location = np.concatenate((lower_location, upper_location, left_location, right_location))
        all_location -= np.array([0.5 * self.edge_length, 0.5 * self.edge_length])  # 重心在原点
        self.initial_config = np.transpose(all_location)

        landmark_location = np.array([[0., 0.], [self.edge_length, 0.], [self.edge_length, self.edge_length],
                                      [0., self.edge_length]])
        landmark_location -= np.array([0.5 * self.edge_length, 0.5 * self.edge_length])
    ...
    def reset_world(self, world):
        ...
        world.landmarks[3].state.p_pos = episodic_landmark_config[3]
    ...
    def reward(self, agent, world):
        ...
        if agent == world.agents[0]:
            ...
            rew -= 1.5*dists[row_ind, col_ind].sum()  # default: 1.0
